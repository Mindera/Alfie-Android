...
class GetFeatureToggleUseCaseTest {

    @MockK
    private lateinit var repository: FeatureToggleRepository

    @InjectMockKs
    private lateinit var subject: GetFeatureToggleUseCase
    ...
    @Test
    fun `Called getFeatureToggle invoke then flow is returned`() = runTest {
        coEvery { repository.getFeatureTogglesByNameAsFlow("Title") } returns mockFeatureToggles

        val result = subject("Title")

        coVerify { repository.getFeatureTogglesByNameAsFlow("Title") }
