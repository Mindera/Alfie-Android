...
Design tokens are sourced from `Mindera/Alfie-Mobile-Design-Tokens` (W3C DTCG format) and committed as JSON under `designsystem/src/main/assets/design_tokens/`. Generated Kotlin lives in `designsystem/src/main/java/com/mindera/alfie/designsystem/tokens/` (package `com.mindera.alfie.designsystem.tokens`).

**Do not edit the generated files by hand.** Re-run the generator whenever tokens change.
...
# 2. Copy updated JSON into the repo
...
# 3. Re-generate Kotlin
python3 scripts/generate_tokens/generate_design_tokens.py

# 4. Verify compilation
./gradlew :designsystem:compileDebugKotlin

# 5. Commit both JSON + generated Kotlin
```

### Using the new design system

```kotlin
// New token facade (Figma vocabulary)
NewTheme.color.surface.backgroundPrimary
NewTheme.typography.display.large
NewTheme.typography.heading.xSmall
NewTheme.spacing.spacing16
NewTheme.sizing.icon.medium
NewTheme.sizing.radius.soft

// Legacy facade (unchanged — do not migrate yet)
Theme.color.primary.mono900
Theme.typography.heading1
```

See `scripts/generate_tokens/README.md` for full documentation.
